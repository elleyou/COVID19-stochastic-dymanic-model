...
for sample_id=1:sample_total
    ...
    N=1.13*10^8; %total population
    
    theta=0.467; %factor for lambda_E=lambda_A=lambda_N*theta;
    lambda_N=0.5604;%infection rate of IN
    lambda_E=lambda_N*theta; %infection rate of the E, same as A
    lambda_A=lambda_N*theta; %infection rate of the A, same as E

    rho=0.6755; %probabiity an exposed person will grow symptom (go to A or E)
    q=0.388; %probability an exposed person is tracible
%     q=0;
    ...
    c=0.1542;
    %c_1=1.1;
    gamma_IH=0.0511*c;%recovery rate of IHL NOT estimated!
    
    
    
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   % S_t=N-150-75-32; %susceptible
    IN0 = 77;
    E0 = fix(220);
    ...
    a=0.0539; %strength of control: all lambda's reduced to lambda*a
...
hold on
a=[32   51   76   96  144  184  236  305  382  509  592  669  777  838  895  965  977  994 1007  995  977  955  927  906  878  845  794 755 708 664 614];
...
plot(30:54, a, 'linewidth',2,'color','k')
...
plot(30:54, a, 'linewidth',2,'color','c')
...
title('Guangdong','FontSize',25)
% title(legend_temp{15},'FontSize',20)
...
legend('recorded accumulated confirmed','recorded standing hospitalized', 'actual accumulated confirmed in the prediction stage','actual standing confirmed in the prediction stage','fitted standing hospitalized CI lower bound',...
    'fitted standing hospitalized CI upper bound','fitted accumulated confirmed CI lower bound',...
...
plot(T_Rc_vector(1:valid_data_length),mean(CI_Rc_bounds_matrix(:,1:valid_data_length)),'b+:','MarkerSize',6, 'linewidth',2);
title('Guangdong','FontSize',25)
% title(legend_temp{15},'FontSize',20)
